		...
		do {
			...
			if (input == 1) {// login
				...
				if (username != null) {// null if username + password do not match
					// create user object
					user = UserService.userInfo(user, username);
				...
				System.out.println("Thank you for using Wu-Tang Financial.\n Goodbye.");
				break;
			} else {
	...
	private static void accountCreator(User user) {
		...
		do {
			...
			if (input == 1) {
				...
				System.out.println("User '" + username + "' has been created: " + UserService.insertUser(user));
				break;
			} else if (input == 2)
				;
