...
public class executeBank {
	...
	public static void startBank() {
		...
		System.out.println("=============================\n Welcome to Big Boy Bank!\n"+"============================= ");

		try	{Thread.sleep(500);
			}catch(InterruptedException e){
			System.out.println("sleep gone wrong");
			}
	...
	static void userLogin(Scanner userInput) {
	...
	static void depositMoney(Customer customer, Scanner userInput) {
		...
		if(deposit.longValue() < 0) {
			...
			realNumber = BankService.addTransaction(customer.getUserName(), deposit);
			if (realNumber == false) {
	...
	static void withdrawMoney(Customer customer, Scanner userInput) {
		...
		if(withdrawl.longValue() < 0) {
			...
			realNumber = BankService.addTransaction(customer.getUserName(), withdrawl);
			if (realNumber == false) {
				System.out.println("Something has gone wrong, please try again.");
			}else {
